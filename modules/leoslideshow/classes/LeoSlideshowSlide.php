...
//		}
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'leoslideshow_slides`
...
//        $id_shop = $context->shop->id;
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_leoslideshow_slides` as id_slide
			FROM `'._DB_PREFIX_.'leoslideshow_slides` hss
	...
	{
		if ($slide_params['navigator_type'] == LeoSlideshowConfig::IVIEW_NAV_BULLET 
				&& $slide_params['enable_custom_html_bullet'] == LeoSlideshowConfig::ENABLE)
		{
			foreach ($sliders as $key => &$slider)
			{
