...
class LeoSlideshowGroup extends ObjectModel
{
	...
	{
		$req = 'SELECT sl.`id_leoslideshow_slides` as id
				FROM `'._DB_PREFIX_.'leoslideshow_slides` sl
				WHERE sl.`id_group` = '.(int)$id_group;
		$sliders = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($req);
		...
		if ($sliders)
		{
			...
			$res &= Db::getInstance()->execute(
					'DELETE FROM `'._DB_PREFIX_.'leoslideshow_slides_lang` 
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
                    DELETE FROM `'._DB_PREFIX_.'leoslideshow_groups`
