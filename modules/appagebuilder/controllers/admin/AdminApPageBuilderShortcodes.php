...
//		$context = Context::getContext();
		...
		foreach ($positions as $item)
		{
			...
			if ($object)
			{
				...
				if ($duplicate_id)
				{
					...
					ApPageSetting::writeFile($position_controller->position_js_folder, $data['position'].$data['position_key'].'.js',
							Tools::file_get_contents($position_controller->position_js_folder.$data['position'].$old_key.'.js'));
					ApPageSetting::writeFile($position_controller->position_css_folder, $data['position'].$data['position_key'].'.css',
							Tools::file_get_contents($position_controller->position_css_folder.$data['position'].$old_key.'.css'));
				}
			}
		}
		if ($is_update)
			...
			if (isset($hook['groups']))
				foreach (self::$language as $lang)
				{
					...
					if ($action == 'export')
					{
						...
						$data[$hook['name']][$lang['iso_code']] = (isset($data[$hook['name']][$lang['iso_code']])
								&& $data[$hook['name']][$lang['iso_code']]) ? $data[$hook['name']][$lang['iso_code']] : '';
	...
	public function saveExceptionConfig($hook, $type, $page, $ids)
	{
		...
		if ($type == 'all')
		{
			if ($type != '')
			{
				...
				{
					$val = trim($val);
					if ($val && (!is_array($this->config_module) || !isset($this->config_module[$hook])
									|| !isset($this->config_module[$hook]['exception'])
									|| !isset($val, $this->config_module[$hook]['exception'])))
	...
	public function getParamByHook($groups, $params, $hook, $action='save')
	{
		...
		foreach ($groups['columns'] as $columns)
		{
			...
			$this->saveExceptionConfig($hook, $columns['params']['specific_type'], $columns['params']['controller_pages'],
					$columns['params']['controller_id']);
			...
			foreach ($columns['widgets'] as $widgets)
			{
				...
				{
					$params .= '['.$widgets['type'].ApShortCodesBuilder::converParamToAttr($widgets['params'],
							$widgets['type'], $this->theme_dir).']';
					foreach ($widgets['widgets'] as $sub_widgets)
					{
						$type_sub = Tools::substr($widgets['type'], 0, -1);
						$params .= '['.$type_sub.ApShortCodesBuilder::converParamToAttr($sub_widgets['params'],
								str_replace('_', '_sub_', $widgets['type']), $this->theme_dir).']';
				...
				else
				{
					...
					elseif ($widgets['type'] == 'ApProductCarousel')
					{
