...
class AdminApPageBuilderPositionsController extends ModuleAdminControllerCore
{
	...
	public function processStatus()
	{
		if (Validate::isLoadedObject($object = $this->loadObject()))
		{
			if ($object->toggleStatus())
			{
				...
				$id_category = (($id_category = (int)Tools::getValue('id_category'))
						&& Tools::getValue('id_product')) ? '&id_category='.$id_category : '';
	...
	public function duplicatePosition($id, $type = '', $name = '')
	{
		...
		if ($object)
		{
			...
			{
				//duplicate shortCode
				ApPageSetting::writeFile($this->position_js_folder, $data['position'].$data['position_key'].'.js',
						Tools::file_get_contents($this->position_js_folder.$data['position'].$old_key.'.js'));
				ApPageSetting::writeFile($this->position_css_folder, $data['position'].$data['position_key'].'.css',
						Tools::file_get_contents($this->position_css_folder.$data['position'].$old_key.'.css'));
				if ($type != 'ajax')
				{
					$this->redirect_after = self::$currentIndex.'&token='.$this->token;
					$this->redirect();
