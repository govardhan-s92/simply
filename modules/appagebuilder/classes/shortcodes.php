...
//			$params = array('token' => Tools::getAdminTokenLite($controller));
		...
		public static function getShortCodeInfos()
		{
			...
			foreach ($source_file as $value)
			{
				...
				if ($fileName == 'index' || $fileName == 'ApColumn' || $fileName == 'ApRow'
						|| ($is_sub_tab && ($fileName == 'ApTabs' || $fileName == 'ApAccordions')))
					continue;
			...
			foreach ($this->config_list as $config)
			{
				...
				if ($config['lang'])
					...
					//$config_val[$config['name']] = Tools::getValue($config['name'], $config['default']);
						$config_val[$config['name']][$lang['id_lang']] = str_replace($this->str_search, $this->str_relace_html_admin,
								Tools::getValue($config['name'].'_'.$lang['id_lang'], $config['default']));
				else if (false !== strpos($config['name'], '[]'))
				{
					...
					$config_val[$config['name']] = str_replace($this->str_search, $this->str_relace_html_admin,
							Tools::getValue($config['name'], $config['default']));
			}
			//$config_val[$config['name']] = Tools::getValue($config['name'], $config['default']);
		...
		protected function appendInputBlockLink($values)
		{
			...
			for ($i = 1; $i <= $total; $i++)
				foreach ($languages as $lang)
					$config_val['title_'.$i.'_'.$lang['id_lang']] = str_replace($this->str_search, $this->str_relace_html_admin,
							Tools::getValue('title_'.$i.'_'.$lang['id_lang']));
			// Get value for link field
			for ($i = 1; $i <= $total; $i++)
				foreach ($languages as $lang)
					$config_val['link_'.$i.'_'.$lang['id_lang']] = str_replace($this->str_search, $this->str_relace_html_admin,
							Tools::getValue('link_'.$i.'_'.$lang['id_lang']));
			$input = array(
				'type' => 'html',
				'name' => 'default_html',
		...
		protected function appendInputSliderCarousel($values)
		{
			...
			foreach ($arr as $i)
				foreach ($inputs as $config)
					foreach ($languages as $lang)
					{
						$config_val[$config][$i][$lang['id_lang']] = str_replace($this->str_search, $this->str_relace_html_admin,
...
//					}
		...
		public function adminContent($atts, $content = null, $tag_name = null, $is_gen_html = null)
		{
			...
			if ($is_gen_html)
			{
				foreach ($atts as $key => $val)
				{
					if (strpos($key, 'content') !== false || strpos($key, 'link') !== false || strpos($key, 'url') !== false
							|| strpos($key, 'alt') !== false || strpos($key, 'tit') !== false
							|| strpos($key, 'name') !== false || strpos($key, 'desc') !== false || strpos($key, 'itemscustom') !== false)
	...
	class ApShortCodesBuilder
	{
		...
		public static function processOverrideTpl($val, $type, $theme_dir)
		{
			...
			if (file_exists(_PS_MODULE_DIR_.'appagebuilder/views/templates/hook/'.$type.'.tpl'))
				Tools::copy(_PS_MODULE_DIR_.'appagebuilder/views/templates/hook/'.$type.'.tpl',
						$theme_dir.'modules/appagebuilder/views/templates/hook/'.$val.'/'.$type.'.tpl');
			else
			{
				$theme_dir_ori = _PS_ROOT_DIR_.'/themes/'.Context::getContext()->shop->getTheme().'/';
				Tools::copy(_PS_MODULE_DIR_.'appagebuilder/views/templates/hook/ApGeneral.tpl',
						$theme_dir_ori.'modules/appagebuilder/views/templates/hook/'.$val.'/'.$type.'.tpl');
			}
		}
