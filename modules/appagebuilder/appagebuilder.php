...
class APPageBuilder extends Module
{
	...
	public function hookPagebuilderConfig($param)
	{
		...
		{
			if (!$this->isCached('config.tpl', $this->getCacheId()))
			{
				$ap_type = $cf;
								
				if($cf == 'profile') $ap_type = 'id_appagebuilder_profiles';
				else if($cf == 'product_builder') $ap_type = 'plist_key';
				$this->smarty->assign(
								array(
			...
			return $this->display(__FILE__, 'config.tpl', $this->getCacheId());
		}
	...
	public function fontContent($assign, $tpl_name)
	{
		...
		if (isset($assign['formAtts']['override_folder'])
			&& file_exists(_PS_ALL_THEMES_DIR_.$this->theme_name.'/modules/'.$this->name.'/views/templates/hook/'.
					$assign['formAtts']['override_folder'].'/'.$tpl_name))
		{
			// validate module
			$tpl_file = 'views/templates/hook/'.$assign['formAtts']['override_folder'].'/'.$tpl_name;
		}
		elseif (file_exists(_PS_ALL_THEMES_DIR_.$this->theme_name.'/modules/'.$this->name.'/views/templates/hook/'.$hook_name.'/'.$tpl_name)
				|| file_exists(dirname(__FILE__).'/views/templates/hook/'.$hook_name.'/'.$tpl_name))
			$tpl_file = 'views/templates/hook/'.$hook_name.'/'.$tpl_name;
		elseif (file_exists(_PS_ALL_THEMES_DIR_.$this->theme_name.'/modules/'.$this->name.'/views/templates/hook/'.$tpl_name)
				|| file_exists(dirname(__FILE__).'/views/templates/hook/'.$tpl_name))
	...
	protected function getCacheId($hook_name = null)
	{
		...
		if ($this->profile_param && isset($this->profile_param[$hook_name]) && $this->profile_param[$hook_name])
		{
			...
			if (isset($this->profile_param[$hook_name]['exception']) && in_array($cache_array, $this->profile_param[$hook_name]['exception']))
				$cache_array[] = $current_page;
		}
	...
	public function setFullwidthHook()
	{
		if (in_array(Context::getContext()->controller->controller_type, array('front', 'modulefront')))
		{
			...
			{
				$this->context->smarty->assign(array(
					'fullwidth_hook' => isset($this->profile_param['fullwidth_index_hook'])
					? $this->profile_param['fullwidth_index_hook'] : ApPageSetting::getIndexHook(3),
				));
			}
			else
			{
				$this->context->smarty->assign(array(
					'fullwidth_hook' => isset($this->profile_param['fullwidth_other_hook'])
					? $this->profile_param['fullwidth_other_hook'] : ApPageSetting::getOtherHook(3),
				));
			}
		}
	}
	...
	public function hookProductOneImg($list_pro)
	{
		...
		foreach ($list_img as $product)
		{
			if (!in_array($product['id_product'], $saved_img))
				$obj[] = array('id' => $product['id_product'], 'content' => ($link->getImageLink($product['link_rewrite'],
						$product['id_image'], $image_name)));
